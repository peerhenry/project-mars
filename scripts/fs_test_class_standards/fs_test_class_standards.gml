/// @arg function
/// @arg args
function fs_test_class_standards(argument0, argument1) {
	var func = argument0;
	var args = argument1;
	var here = fs_test_class_standards;

	show_debug_message("fs_test_class_standards." + func);
...
#region test_run_method
	case "test_run_method":
		...
		var rt = meth.type_info.return_type_info;
		var params = meth.type_info.parameters;
		...
		show_debug_message("test_run_method: " + script_get_name(instance.class) + "." + meth.name + "(" + param_types_string + ")");
	
		// create dummy parameters
		var dummy_params = morph(param_type_infos, "create_dummy");
		var result = call(instance, meth.name, dummy_params);
		var expect_exception = meth.type_info.throws_exception;
		var is_ok = STATUS.OK == result.status;
	
		// assert result
		if(expect_exception)
		{
			assert_false(is_ok, "result from calling " + meth.name + " should not be OK");
			instance_destroy(result);
		}
		else assert_true(is_ok, "result from calling " + meth.name + " should be OK");
