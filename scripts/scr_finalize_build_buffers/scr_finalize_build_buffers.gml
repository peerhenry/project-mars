if(global.debug_start_messages){
show_debug_message("");
...
for(var s = 0; s< ds_list_size(global.construction_list); s++)
{
	...
	if(global.debug_start_messages) show_debug_message("next key from build_tile_counts: " + string(next_construction )); // DEBUG
	var nr_of_tiles = ds_map_find_value(global.build_tile_counts, next_construction );
	if(global.debug_start_messages) show_debug_message("number of tiles for this: " + string(nr_of_tiles)); // DEBUG
...
{
	var next_construction = ds_list_find_value(global.construction_list, s);
	if(global.debug_start_messages) show_debug_message("#next construction: " + string(next_construction)); // DEBUG
	var nr_of_tiles = ds_map_find_value(global.build_tile_counts, next_construction);
	if(global.debug_start_messages) show_debug_message("#number of tiles for this: " + string(nr_of_tiles)); // DEBUG
	var tile_offset = buffer_tell(global.build_tile_buffer);
	...
	for(var n = 0; n < nr_of_tiles; n++ )	// loops over build tiles
	{
		...
		{
			var construction = buffer_read(global.big_bad_buffer, buffer_u32);
			var d_i = buffer_read(global.big_bad_buffer, buffer_s32);
			...
			if(m == 0){ // add a build tile for each first action
				...
				buffer_write(global.build_tile_buffer, buffer_s32, d_i);
				buffer_write(global.build_tile_buffer, buffer_s32, d_j);
				buffer_write(global.build_tile_buffer, buffer_u32, action_offset);
				buffer_write(global.build_tile_buffer, buffer_u32, nr_of_actions);
				if(global.debug_start_messages) show_debug_message("just wrote to build_tile_buffer: "); // DEBUG
				if(global.debug_start_messages) show_debug_message(string(d_i) + ", " + string(d_j) + " , " + string(action_offset) + " , " + string(nr_of_actions) ); // DEBUG
			}
			
			if(global.debug_start_messages) show_debug_message("action for : " + string(construction) + " delta: " + string(d_i) + ", " + string(d_j) ); // DEBUG
