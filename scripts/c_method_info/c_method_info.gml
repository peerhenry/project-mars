function c_method_info(argument0, argument1, argument2) {
	var func = argument0;
	var this = argument1;
	var args = argument2;
	...
	#region constructor
		...
		case constructor: // can be remove if not needed
			...
			foreach(this.parameters, this, "validate_parameter");
	...
	#region methods
		...
		case "copy":
			...
			var copy = fnew(c_method_info, [return_info_copy, param_copies]);
			return ok(copy);
		...
		case "assert_type": // given an instance and a method, does it respond to giver arguments - and does it return a proper type
			...
			var returned = call_unwrap(instance, func, dummy_args);
	...
	#region tests
		...
		case "test_construct":
			// setup
			var thing = fnew(c_method_info, t_string());
			// assert
		...
		case "test_assert_match":
			// setup
			var substrate = fnew(c_method_info, [
				t_void(), 
				[p_number("d1"), p_string("d2"), p_object("d3", obj_empty)]
			]);
			var thing = fnew(c_method_info, [
		...
		case "test_private_param_to_dummy":
			// setup
			var param = fnew(c_parameter, ["sam", t_object(obj_empty)]);
			// act
		...
		case "test_assert_type":
			var meth_info = fnew(c_method_info, [
				t_void(), 
				[p_number("d1"), p_string("d2"), p_object("d3", obj_empty)]
			]);
			var mocky = fnew(c_mock);
			show_debug_message("1 obj_empty count: " + string(scr_count_instances(obj_empty)));
			var prop = fnew(c_class_property, ["foo", meth_info]);
