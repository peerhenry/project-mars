function test_step_sleep_and_speed() {
	test_init(test_step_sleep_and_speed);

	var astro = instance_create_depth(0,0,0,obj_astronaut);
	var energy_before = astro.energy;
	var expected_energy = energy_before - global.sleep_level_depletion;
	var expected_speed = scr_calculate_movement_speed(expected_energy);

	// act
	scr_step_sleep_and_speed(astro);

	// assert
	var energy_after = astro.sleep_level;
