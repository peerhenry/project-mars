/// @descr Mock class 
function c_mock(argument0, argument1, argument2) {
	var func = argument0;
	var this = argument1;
	var args = argument2;
	var here = c_mock;
	...
	#region call_stub
		...
		case "call_stub":
			...
			{
				var action = fnew(c_action, [method_info, "assert_arguments", call_args]);
	...
	#region tests
		...
		case "test_add_prop":
			...
			var m = fnew(c_mock);
			// act
			var prop = fnew(c_class_property, ["jim", t_number()]);
			call_unwrap(m, "add_prop", prop);
			// assert
			var dti = t_number();
		...
		case "test_call_stub":
			...
			var expect = 25;
			// act
			var prop = fnew(c_class_property, [
				"foo", fnew(c_method_info, t_number())
			]);
			call_unwrap(m, "add_prop", prop);
		...
		case "test_verify_last_call_arguments":
			...
			call_unwrap(mock, "add_prop", prop);
			call_unwrap(mock, "setup_stub", ["foo", ok(expect)]);
			var res = call_unwrap(m, "foo", [1,2,3]);
			// assert
			assert_equal(expect, res, "result from setup_stub");
			call_unwrap(mock, "verify_last_call_arguments", [
				"foo", 
				[1,2,3]
			]);
			// cleanup
		...
		default:
			if(!is_undefined(this) && this != noone) return call(this, "call_stub", [func, args]);
			else return refused();
	}
