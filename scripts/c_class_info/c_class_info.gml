/// @descr Mock class 
function c_class_info(argument0, argument1, argument2) {
	var func = argument0;
	var this = argument1;
	var args = argument2;
	...
	#region methods
		case "copy":
			...
			var copy = fnew(here, copied_props);
			return ok(copy);
	
		case "create_dummy":
			var m = fnew(c_mock, this);
			return ok(m);
	...
	#region tests
		...
		case "test_constructor_empty":
			var thing = fnew(c_class_info);
			destroy(thing);
			break;
	
		case "test_constructor":
			var thing = fnew(c_class_info, [
				fnew(c_class_property, ["dummy_number", t_number()])
			]);
		...
		case "test_get_data_props":
			...
			var thing = fnew(c_class_info, [
				fnew(c_class_property, [dummy_number, t_number()]),
				fnew(c_class_property, ["dummy_method", fnew(c_method_info)])
			]);
			// act
		...
		case "test_get_method_props":
			...
			var thing = fnew(c_class_info, [
				fnew(c_class_property, [dummy_number, t_number()]),
				fnew(c_class_property, [dummy_meth, fnew(c_method_info)])
			]);
			// act
		...
		case "test_extract_dependency":
			var dep_name = "deppy";
			var expect = fnew(c_class_info);
			var thing = fnew(c_class_info, [
				fnew(c_class_property, [dep_name, expect])
			]);
			// act
		...
		case "test_extract_dependency_from_method":
			...
			var expect = fnew(c_class_info);
			var thing = fnew(c_class_info, [
				prop_method("foo", t_void(), fnew(c_parameter, [dep_name, expect])) 
			]);
			// assert setup
		...
		case "test_create_dummy":
			var method_info = fnew(c_method_info, [ 
				t_void(), 
				fnew(c_parameter, [
					"dep_name", 
					t_number()
			...
			var thing = fnew(c_class_info, [
				fnew(c_class_property, ["foo", method_info]),
				fnew(c_class_property, ["numby", t_number()])
			]);
			// act
		...
		case "test_assert_match":
			...
			var thing = fnew(c_class_info, [
				fnew(c_class_property, [match_prop, t_number()])
			]);
			var bigger = fnew(c_class_info, [
				fnew(c_class_property, [match_prop, t_number()]),
				fnew(c_class_property, ["something else", t_string()])
			]);
			// act/assert
		...
		case "test_assert_type":
			var thing = fnew(c_class_info, [
				fnew(c_class_property, ["billy", t_number()]),
				fnew(c_class_property, ["something else", t_string()])
			]);
			var mocky = fnew(c_mock, thing);
			// assert
