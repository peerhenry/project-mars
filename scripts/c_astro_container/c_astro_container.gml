function c_astro_container(argument0, argument1, argument2) {
	...
	#region METHODS
		...
		case "eject":
			...
			if(result.status != STATUS.OK)
			{
				...
				// make eject action
				var eject = fnew(c_action, [this, "eject"]);
				// put it in a decorator
				var dec = fnew(c_action_decorator, eject);
				// append destruction of the eject action
				dec = call_unwrap(dec, "then", fnew(c_action_script, [destroy, eject]));
				var delayed = fnew(c_delay, [dec, 5]); // try again after this many seconds
