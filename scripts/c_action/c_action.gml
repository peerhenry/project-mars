...
function c_action(argument0, argument1, argument2) {
	var func = argument0;
	var this = argument1;
	var args = argument2;
	var here = c_action;
	...
	switch(func)
	{
		...
		case "execute":
			if(scr_length(args) == 1)
			{
				...
				return call(this.instance, this.func, final_args);
			}
			return call(this.instance, this.func, this.action_arguments);
	
		case "execute_destroy":
			call_unwrap(this.instance, this.func, this.action_arguments);
			destroy(this);
			return ok();
		...
		case "test_execute_with_args":
			...
			var args = [1,2,"five"];
			var item = fnew(here, [mocky, stub_name, args]);
			// act
