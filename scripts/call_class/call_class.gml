...
/// @arg method
/// @arg arguments
function call_class(argument0, argument1, argument2, argument3) {
	var class = argument0;
...
#region trace call
	var trace_it = debug_mode && class != c_result && func != get_object_index;
	if(trace_it)
	{
		...
		show_debug_message("[CALL] " + script_get_name(class) + "." + func + "(" + argy + ")");
	}
#endregion trace call

	var result = script_execute(class, func, instance, args);
	if(result.refused_request)
	{
		instance_destroy(result);
		var parent = cs_get_parent_class(class);
		result = call_class(parent, instance, func, args);
	}
	else
	{
		if(func == get_object_index && result.value == noone) scr_panic("result.value is noone in get_object_index for " + script_get_name(class));
	}
