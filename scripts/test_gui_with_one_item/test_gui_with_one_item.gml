function test_gui_with_one_item() {
	...
	// act
	var item = instance_create_depth(gui, 0, 0, obj_gui_item);

	// assert
	assert_list_contains(gui.gui_item_list, item);
	assert_equal(1, ds_list_size(gui.gui_item_list), "gui_item_list size");
	assert_equal(1, scr_count_instances_strict(obj_gui_item), "count obj_gui_item");
	assert_equal(0, scr_count_instances_strict(obj_gui_container), "count obj_gui_container");
